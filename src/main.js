  ...
  constructor(player, options) {
    ...
    if ('autoplay' in this.player.attributes) {
      ...
      if (promise !== undefined) {
        promise
          .then(() => {
            const playPauseButton = self.player.parentElement.querySelector(
        ...
        window.addEventListener(
          ...
          () => {
            if (
              ...
              self.currentlyDragged.parentElement.parentElement ===
                self.sliders[0] &&
              self.paused !== self.player.paused
            ) {
              self.togglePlay();
